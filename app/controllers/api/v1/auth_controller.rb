...
def login
    ...
    if user && user.authenticate(params[:password]) #.authenticate method to check password using bcrypt gem
        ...
        render json: {user: user, daily_logs: user.daily_logs, email: user.email, id: user.id, name: user.name, weight: user.weight, bank: user.bank, image: user.image, city: user.city, 
