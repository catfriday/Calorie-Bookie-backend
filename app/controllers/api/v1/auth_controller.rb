...
def login
    ...
    if user && user.authenticate(params[:password]) #.authenticate method to check password using bcrypt gem
        ...
        render json: { monthly_progress: user.logged, daily_logs: user.daily_logs, email: user.email, id: user.id, name: user.name, weight: user.weight, bank: user.bank, image: user.image, city: user.city, calories: user.calories, 
