class Api::V1::UsersController < ApplicationController
  ...
  def show 
    ...
    render json: user,  methods:[:logged, :todays_calories], except: [:created_at, :updated_at], include: [:bets=>{except: [:created_at, :updated_at], methods: [:win_or_lose] }, :daily_logs =>{except: [:created_at, :updated_at], include: [:food_items => {except: [:created_at, :updated_at]}], methods: [:calories, :daily_goal_reached]}]
